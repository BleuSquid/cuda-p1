...
void E_init_d(double *g, double value, int n) {
	...
	cutilSafeCall(cudaMemset(g, 0, sizeof(double) * n));
...
int init_ffts() {
	...
	int first_found = 0;
#define COUNT 149
	int default_mult[COUNT] = {  //this batch from GTX780 and RTX2070 timings
		2, 4, 14, 16, 18, 32, 36, 48, 50, 54, 64, 70, 72, 80, 96, 100, 112, 128, 144,
		160, 162, 192, 196, 200, 216, 224, 256, 288, 320, 324, 336, 360, 384, 392,
		400, 432, 448, 512, 540, 560, 576, 640, 648, 672, 720, 800, 864, 1024, 1080,
		1120, 1134, 1152, 1280, 1296, 1344, 1350, 1400, 1440, 1458, 1512, 1568, 1600,
		1728, 1792, 2048, 2304, 2560, 2592, 2700, 2744, 2880, 2916, 3136, 3200, 3402,
		3456, 4096, 4608, 4800, 5120, 5184, 5600, 5760, 5832, 6048, 6144, 6272, 6400,
		6912, 7168, 7200, 8192, 8640, 8748, 9216, 9408, 9600, 10240, 10368, 10584,
		11200, 11520, 11664, 12096, 12288, 12544, 12800, 13824, 14336, 14400, 15552,
		16384, 17496, 18432, 20480, 20736, 21168, 21952, 23040, 23328, 24192, 24576,
		25088, 25600, 26244, 27648, 28672, 28800, 32768, 34992, 36288, 36864, 38880,
		40960, 41472, 43200, 43904, 44800, 46656, 48384, 49152, 51200, 52488, 55296,
		57344, 57600, 58320, 61440, 65536};
...
int standardize_digits_int(int *x_int, int q, int n, int offset, int num_digits) {
	...
	digit = (int) floor(offset * (n / (double) q));
	j = (n + digit - 1) % n;
...
int printbits_int(int *x_int, int q, int n, int offset, FILE* fp, char *expectedResidue, int o_f) {
	...
	digit = (int) floor(offset * (n / (double) q));
	bit = offset - (int) ceil(digit * (q / (double) n));
	j = digit;
	while (k < 64) {
		temp = x_int[j];
...
int stage2(int* x_int, unsigned* x_packed, int q, int n, int nrp, float err) {
...

	{
		...
		for (d = start_d; d > 1; d /= d_div) {
			...
			if (b1 * sprimes[i] * 53 < b2)
				...
				ks = ((((b1 + 1) >> 1) + d - 1) / d - 1) * d;
			ke = ((((b2 + 1) >> 1) + d - 1) / d) * d;
			max_ke = ke > max_ke ? ke : max_ke;
			ks = ((ks / d) << 1) + 1;
			ke = (ke / d) << 1;

			for (e = start_e; e <= end_e || best_d == 0; e += 2) {
				nrpe = nrp - e - 1;
				if (nrpe <= 0)
					break;
				...
				guess += e * (ke - ks) * passes + (int)((2.2) * (numprimes - numpairings));
				if (e == 4)
					guess = (int)((float)guess * (float)0.87);
				if (e == 6)
					guess = (int)((float)guess * (float)0.82);
				if (e == 12)
					guess = (int)((float)guess * (float)0.75);
				if (guess < best_guess) {
					best_guess = guess;
					best_d = d;
					best_e = e;
	...
	if (d == 0) {
		// We need a better explanation for why we abort here
		// It seems we should not end up here unless there are overflows in the above calculations
		printf("Exiting, d = 0\n");
		exit(3);
	}
	...
	max_ke = ke > max_ke ? ke : max_ke;
	bprimes = (uint8*) malloc(max_ke * sizeof(uint8));
		...
		for (; k < ke && !quitting; k += 2) {
			...
			if ((k == next_checkpoint || quitting == 1) && quitting != 2) {
				...
				if (err <= 0.4f) {
					...
					print_time_from_seconds(stdout,
							(int) (itime * ((double) itran_tot / (double) itran_done - 1)
									+ ptime * ((double) ptran_tot / (double) (ptran_done + num_tran) - 1)));
					printf(")\n");
					fflush (stdout);
					tran_save = num_tran;
					time0.tv_sec = time1.tv_sec;
...
int check_pm1(int q, char *expectedResidue) {
	...
	do { /* while (restarting) */
		...
		g_d = g_d_commandline;
		if (g_nrp == 0)
			nrp = (int) ((free_mem - (size_t) unused_mem * 1024 * 1024) / n / 8 - 7);
#ifdef ENV32BIT
		...
		for (; !restarting && j <= last; j++)  // Main LL loop
			...
			if ((j % checkpoint_iter) == 0 || quitting) {
				...
				if (quitting == 0) {
					...
					long long diff2 = (long long) ((last - j) * diff1 / ((checkpoint_iter - j_resume) * 1e6));
					gettimeofday(&time0, NULL);
					...
					if (j_resume)
						j_resume = 0;
					reset_err(&maxerr, (float) 0.85);  // Instead of tracking maxerr over whole run, reset it at each checkpoint.
				} else {
...
void parse_args(int argc, char *argv[], int* q, int* device_number, int* cufftbench_s, int* cufftbench_e, int* cufftbench_d) {
	while (argc > 1) {
		if (strcmp(argv[1], "-t") == 0) {
			...
			// we already print the program version as the very first thing, so just exit
			// printf("%s\n\n", program);
			exit(2);
		} else if (strcmp(argv[1], "-V") == 0) {
			verbose = 1;
